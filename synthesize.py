...
if __name__ == "__main__":
  if len(sys.argv) <= 1:
    print 'use "python synthesize.py <output_dir>" to generate SLS load files'
    exit()
  ...
  maxJobAccumulatedUsageRatio = 0.5
  minJobAccumulatedUsageRatio = 0.1

  # Task runtime
  minTaskRuntime = totalTime * 0.1
  maxTaskRuntime = totalTime * 0.5
  absMinTaskRuntime = 1 * MIN
  ...
  while restAccumulatedRes > 0.05 * accumulatedResUpperBound:
    ...
    jobAccumulatedRes = min(restAccumulatedRes, accumulatedResUpperBound
                            * random.uniform(minJobAccumulatedUsageRatio, maxJobAccumulatedUsageRatio) * queueCapacities[queue])
